...
// ================================================================
// AUDIO ENGINE — Música procedural electrónica + SFX
// Usa Web Audio API. Se inicializa de forma lazy al primer gesto
// del usuario para cumplir las políticas de autoplay del navegador.
// No requiere archivos externos: todo se genera matemáticamente.
// ================================================================
class AudioEngine {
  constructor() {
    this.actx     = null;
  ...
  /* Crea el AudioContext la primera vez (requiere gesto del usuario). */
  _boot() {
    if (this.actx) {
    ...
    if (!Ctx) return;
    this.actx = new Ctx();
    // Compresor maestro: evita clipping cuando varios osciladores suenan juntos
    ...
    this.master = this.actx.createGain(); this.master.gain.value = 0.7;
    this.master.connect(comp);
    this.musicBus = this.actx.createGain(); this.musicBus.gain.value = 0.48;
    this.musicBus.connect(this.master);
    this.sfxBus = this.actx.createGain(); this.sfxBus.gain.value = 0.88;
    this.sfxBus.connect(this.master);
  }

  /* ── MÚSICA ─────────────────────────────────────────────────── */
  startMusic() {
    this._boot(); if (!this.actx || this._playing) return;
    this._playing = true; this._step = 0;
    this._nextStep = this.actx.currentTime + 0.05;
    this._scheduler();
  }
  stopMusic() { this._playing = false; clearTimeout(this._timer); }

  /* Planificador adelantado: programa notas antes de que llegue su tiempo
     para evitar glitches causados por garbage collection o callbacks tardíos. */
  _scheduler() {
    if (!this._playing) return;
    const s16 = 60 / 126 / 4; // duración de semicorchea a 126 BPM
    while (this._nextStep < this.actx.currentTime + 0.12) {
      this._step16(this._nextStep, this._step);
      this._nextStep += s16;
      this._step = (this._step + 1) % 32; // patrón de 2 compases = 32 pasos
    }
  ...
  _step16(t, step) {
    const s16 = 60 / 126 / 4;
    // Escala pentatónica menor de A: A3, C4, D4, E4, G4, A4, C5
    const SC = [220, 261.63, 293.66, 329.63, 392, 440, 523.25];

    // BOMBO — en cada tiempo (cada 4 pasos)
    if (step % 4 === 0) this._kick(t, (step === 0 || step === 16) ? 1.0 : 0.82);

    // CAJA — contratiempos 2 y 4 de cada compás (pasos 4,12,20,28)
    if (step===4||step===12||step===20||step===28) this._snare(t);

    // HI-HAT CERRADO — en los pasos de corchea pares intermedios
    if (step % 4 === 2) this._hihat(t, 0.3);

    // HI-HAT ABIERTO — en el "4 and" de cada compás
    if (step === 14 || step === 30) this._hihat(t, 0.55);

    // LÍNEA DE BAJO — repite cada 16 pasos
    const BP = [0,-1,2,-1, 0,-1,4,-1, 3,-1,2,-1, 0,-1,-1,-1];
    const bi = BP[step % 16];
    if (bi >= 0) this._bass(t, SC[bi] / 2, s16 * 1.7);

    // MELODÍA LEAD — patrón de 32 pasos (2 compases)
    const LP = [
      4,-1,-1,2, 5,-1,4,-1, 2,-1,5,-1, 4,-1,-1,-1,
      6,-1,5,-1, 4,-1,2,-1, 0,-1,2,-1, 4,-1,-1,-1,
    ];
    const li = LP[step];
    if (li >= 0) this._lead(t, SC[li], s16 * 1.5);

    // PAD DE ACORDE — en el inicio de cada compás (pasos 0 y 16)
    if (step === 0 || step === 16) {
      const root = step === 0 ? SC[0] : SC[4] / 2;
      [root, root * 1.189, root * 1.498].forEach(f => this._pad(t, f, s16 * 16));
    }
  }
  ...
  _kick(t, vol=1) {
    ...
    o.frequency.setValueAtTime(200,t); o.frequency.exponentialRampToValueAtTime(40,t+0.14);
  ...
  _snare(t) {
    ...
    this._noise(t,0.1,0.35,2000,this.musicBus);
  }
  _hihat(t, vol=0.35) { this._noise(t,0.06,vol,8000,this.musicBus); }
  _bass(t, freq, dur) {
    const o=this.actx.createOscillator(), f=this.actx.createBiquadFilter(), g=this.actx.createGain();
    o.type='sawtooth'; o.frequency.value=freq;
    f.type='lowpass'; f.frequency.setValueAtTime(700,t);
    f.frequency.exponentialRampToValueAtTime(150,t+dur*0.6); f.Q.value=3;
    g.gain.setValueAtTime(0.42,t); g.gain.exponentialRampToValueAtTime(0.001,t+dur);
    o.connect(f); f.connect(g); g.connect(this.musicBus); o.start(t); o.stop(t+dur+0.02);
  }
  _lead(t, freq, dur) {
    const o=this.actx.createOscillator(), f=this.actx.createBiquadFilter(), g=this.actx.createGain();
    o.type='square'; o.frequency.value=freq;
    f.type='lowpass'; f.frequency.value=2400; f.Q.value=2;
    g.gain.setValueAtTime(0.001,t); g.gain.linearRampToValueAtTime(0.13,t+0.015);
  ...
  _pad(t, freq, dur) {
    ...
    g.gain.setValueAtTime(0.05,t+dur-0.3); g.gain.exponentialRampToValueAtTime(0.001,t+dur);
    o.connect(g); g.connect(this.musicBus); o.start(t); o.stop(t+dur+0.02);
  }
  _noise(t, dur, vol, hp, dest) {
    const ctx=this.actx;
    const n=Math.floor(ctx.sampleRate*(dur+0.01));
    ...
    g.connect(dest||this.musicBus); src.start(t); src.stop(t+dur+0.02);
  }

  /* ── SFX ────────────────────────────────────────────────────── */
  /* Laser digital al disparar */
  shoot() {
    this._boot(); if(!this.actx) return;
    const ctx=this.actx, t=ctx.currentTime;
    const o=ctx.createOscillator(), g=ctx.createGain();
    o.type='square';
    o.frequency.setValueAtTime(1100,t); o.frequency.exponentialRampToValueAtTime(180,t+0.09);
  ...
  /* Golpe grave al recibir daño */
  hit() {
    this._boot(); if(!this.actx) return;
    const ctx=this.actx, t=ctx.currentTime;
    const o=ctx.createOscillator(), g=ctx.createGain();
    o.type='sine';
    o.frequency.setValueAtTime(160,t); o.frequency.exponentialRampToValueAtTime(35,t+0.24);
    g.gain.setValueAtTime(0.72,t); g.gain.exponentialRampToValueAtTime(0.001,t+0.28);
    o.connect(g); g.connect(this.sfxBus); o.start(t); o.stop(t+0.3);
    this._noise(t,0.05,0.45,300,this.sfxBus);
  }
  /* Arpegio ascendente al desbloquear personaje */
  unlock() {
    this._boot(); if(!this.actx) return;
    const ctx=this.actx, t=ctx.currentTime;
    [523.25,659.25,783.99,1046.5].forEach((freq,i)=>{
      const o=ctx.createOscillator(), g=ctx.createGain();
      o.type='sine'; o.frequency.value=freq; const st=t+i*0.1;
      g.gain.setValueAtTime(0.28,st); g.gain.exponentialRampToValueAtTime(0.001,st+0.45);
      o.connect(g); g.connect(this.sfxBus); o.start(st); o.stop(st+0.5);
    });
  }
  /* Tres tonos descendentes en game over */
  gameOver() {
    this._boot(); if(!this.actx) return;
    const ctx=this.actx, t=ctx.currentTime;
    [350,280,180].forEach((freq,i)=>{
      const o=ctx.createOscillator(), g=ctx.createGain();
      o.type='sawtooth'; o.frequency.value=freq; const st=t+i*0.2;
      g.gain.setValueAtTime(0.22,st); g.gain.exponentialRampToValueAtTime(0.001,st+0.5);
      o.connect(g); g.connect(this.sfxBus); o.start(st); o.stop(st+0.55);
    });
  }
  /* Pitido digital al activar/reconfigurar obstáculos */
  obstacleSpawn() {
    this._boot(); if(!this.actx) return;
    const ctx=this.actx, t=ctx.currentTime;
    const o=ctx.createOscillator(), g=ctx.createGain();
    o.type='sawtooth';
    o.frequency.setValueAtTime(100,t); o.frequency.exponentialRampToValueAtTime(700,t+0.07);
  ...
  /* Impacto seco cuando una bala choca contra un obstáculo */
  wallHit() {
    this._boot(); if(!this.actx) return;
    this._noise(this.actx.currentTime,0.04,0.22,2500,this.sfxBus);
  }
  /* Alterna silencio/sonido */
  toggleMute() {
    this.muted = !this.muted;
    if(this.master) this.master.gain.setTargetAtTime(this.muted?0:0.7,this.actx.currentTime,0.06);
    return this.muted;
  }
}
...
class Game {
  ...
  _bindButtons() {
    ...
    this.ui.muteBtn.addEventListener('click', () => {
      ...
      this.ui.muteIcon.textContent = muted ? '\u266A OFF' : '\u266A ON';
